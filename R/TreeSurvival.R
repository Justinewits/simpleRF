...
methods = list(
  ...
  findBestSplitValueOrdered = function(split_varID, data_values, best_split, response) {
    ...
    for (j in 1:length(possible_split_values)) {
      ...
      if (splitrule == "Logrank") {
      ...
      } else if (splitrule == "sampled"){
        
        ## Skip if one child empty
        if (sum(idx) < 2 | sum(!idx) < 2) {
          next
        }
        ...
        #time_child1 <- time[idx]
        #time_child2 <- time[!idx]
        #cstatus_child1<-cstatus[idx]
        #cstatus_child2<-cstatus[!idx]
        group <- ifelse(idx, "1", "2")
        df <- data.frame(time = time, status = cstatus, group)
        df <- df[order(df$time), ]
        event_times <- unique(df$time[df$status == 1])
        obs_events <- tapply(df$status, as.character(df$group), sum, na.rm = TRUE)
        unique_groups <- unique(as.character(df$group))
        exp_events <- setNames(rep(0, length(unique_groups)), unique_groups)
        for (t in event_times) {
          risk_set <- df[df$time >= t, ]
          if (nrow(risk_set) > sample_size) {
            risk_set <- risk_set[sample(1:nrow(risk_set), sample_size), ]
          }
          at_risk <- table(as.character(risk_set$group))
          total_at_risk <- sum(at_risk)
          d_t <- sum(df$status[df$time == t])
          for (g in unique_groups) {
            if (g %in% names(at_risk)) {
              exp_events[g] <- exp_events[g] + (at_risk[g] / total_at_risk) * d_t
            }
          }
        }
        O_minus_E <- obs_events - exp_events
        V <- sum(exp_events * (1 - exp_events / sum(exp_events)))
        
        if (V == 0) {
          next
        }
        
        chi_sq  <- sum((O_minus_E^2) / V)
        teststat <-chi_sq 
        if(is.na(teststat)) {
          browser()
        }
